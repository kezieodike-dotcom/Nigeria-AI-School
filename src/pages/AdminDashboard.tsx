...
const tabs = [
  ...
  { name: 'Applications', icon: UserCheck },
  { name: 'Videos', icon: Video },
  { name: 'Courses', icon: BookOpen },
  { name: 'Payments', icon: CreditCard },
  { name: 'Subscriptions', icon: Activity },
...
function StatusBadge({ status }: { status?: string | null }) {
  ...
  return (
    <span className={cn('rounded-lg px-2.5 py-1 text-[10px] font-black uppercase tracking-widest', statusStyles[normalized] || statusStyles.inactive)}>
      {normalized}
    </span>
  );
}
...
function EmptyState({ icon: Icon, message }: { icon: React.ElementType; message: string }) {
  return (
    <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-outline-variant/20 bg-white py-16 text-center">
...
export default function AdminDashboard() {
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4">
        {metricCards.map((stat) => (
          <GlowCard key={stat.label} glowColor={stat.glow} className="h-auto rounded-3xl border border-outline-variant/10 bg-white p-6">
            ...
            <h3 className="font-headline text-3xl font-black tracking-tight text-primary">{stat.value}</h3>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <div className="rounded-3xl border border-outline-variant/10 bg-white p-6 xl:col-span-2">
          <div className="mb-5 flex items-center justify-between">
            <h2 className="font-headline text-xl font-bold text-primary">Recent Users</h2>
            <button onClick={() => setActiveTab('Users')} className="text-sm font-black text-primary hover:underline">View all</button>
          </div>
          {filteredUsers.length === 0 ? (
            ...
            <div className="divide-y divide-outline-variant/10">
              {filteredUsers.slice(0, 5).map((person) => (
                <div key={person.id} className="flex items-center justify-between gap-4 py-4">
        ...
        <div className="rounded-3xl border border-outline-variant/10 bg-white p-6">
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3">
          ...
          <input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search users" className="bg-transparent text-sm outline-none" />
        </div>
      </div>
      {filteredUsers.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-3xl border border-outline-variant/10 bg-white">
          {filteredUsers.map((person) => (
            <div key={person.id} className="grid gap-3 border-b border-outline-variant/10 p-5 last:border-b-0 md:grid-cols-[1.5fr_0.8fr_1fr_0.9fr] md:items-center">
              <div className="flex min-w-0 items-center gap-3">
                <img
                  src={person.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(person.name)}&background=1E40AF&color=fff`}
                  alt={person.name}
              ...
              <div className="flex justify-start md:justify-end">
                {person.role === 'student' ? (
                  <button
                    ...
                    className="inline-flex min-w-[120px] items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2 text-xs font-black text-white disabled:opacity-60"
                  >
      ...
      {creatorApplications.length === 0 ? (
        ...
        <div className="space-y-4">
          {creatorApplications.map((application) => (
            <div key={application.id} className="rounded-3xl border border-outline-variant/10 bg-white p-5 md:p-6">
              <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <h3 className="font-headline text-lg font-bold text-primary">{application.full_name}</h3>
                ...
                {application.status === 'pending' && (
                  <div className="flex shrink-0 gap-2">
                    <button onClick={() => reviewApplication(application.id, 'approve')} disabled={reviewingApplicationId === application.id} className="rounded-xl bg-green-600 px-4 py-2 text-xs font-black text-white disabled:opacity-60">Approve</button>
                    <button onClick={() => reviewApplication(application.id, 'reject')} disabled={reviewingApplicationId === application.id} className="rounded-xl bg-red-50 px-4 py-2 text-xs font-black text-red-600 disabled:opacity-60">Reject</button>
                  </div>
                )}
              </div>
            </div>
  ...
  const renderCourses = () => (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <button onClick={() => setShowUploadModal(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-black text-white">
      ...
      {courses.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
          {courses.map((course) => (
            <GlowCard key={course.id} glowColor="purple" className="h-auto overflow-hidden rounded-3xl bg-white">
              ...
              <div className="space-y-4 p-5">
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div className="rounded-2xl bg-surface-container-low p-3"><p className="text-xs font-black text-primary">{course.students || 0}</p><p className="text-[9px] font-bold uppercase text-on-surface-variant">Students</p></div>
                  <div className="rounded-2xl bg-surface-container-low p-3"><p className="text-xs font-black text-primary">{course.views || 0}</p><p className="text-[9px] font-bold uppercase text-on-surface-variant">Views</p></div>
                  <div className="rounded-2xl bg-surface-container-low p-3"><p className="text-xs font-black text-primary">{course.average_progress || 0}%</p><p className="text-[9px] font-bold uppercase text-on-surface-variant">Progress</p></div>
                </div>
  ...
  const renderVideos = () => (
    <div className="space-y-5">
      ...
      {videos.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
          {videos.map((course) => (
            <GlowCard key={course.id} glowColor="blue" className="h-auto rounded-3xl border border-outline-variant/10 bg-white p-5">
      ...
      {payments.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-3xl border border-outline-variant/10 bg-white">
          {payments.map((payment) => (
            <div key={payment.id} className="grid gap-3 border-b border-outline-variant/10 p-5 last:border-b-0 lg:grid-cols-[1.4fr_1fr_1fr_1fr] lg:items-center">
              <div><p className="font-black text-primary">{payment.student_name}</p><p className="text-xs text-on-surface-variant">{payment.student_email}</p></div>
      ...
      {subscriptions.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-3xl border border-outline-variant/10 bg-white">
          {subscriptions.map((subscription) => (
            <div key={subscription.id} className="grid gap-3 border-b border-outline-variant/10 p-5 last:border-b-0 lg:grid-cols-[1.4fr_1fr_1fr_1fr] lg:items-center">
              <div><p className="font-black text-primary">{subscription.student_name}</p><p className="text-xs text-on-surface-variant">{subscription.student_email}</p></div>
  ...
  const renderPayouts = () => (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <button onClick={() => setShowPayoutModal(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-black text-white">
      ...
      {payouts.length === 0 ? (
        ...
        <div className="grid gap-4">
          {payouts.map((payout) => (
            <div key={payout.id} className="grid gap-3 rounded-3xl border border-outline-variant/10 bg-white p-5 md:grid-cols-[1.4fr_1fr_1fr_1fr] md:items-center">
              <p className="font-black text-primary">{payout.creator_name}</p>
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      <div className="rounded-3xl border border-outline-variant/10 bg-white p-6">
        ...
        <div className="mt-5 space-y-3 text-sm">
          ...
          <p><strong className="text-primary">Admin emails:</strong> mvpxlab@gmail.com, kezieodike@gmail.com</p>
  ...
  return (
    <div className="flex min-h-screen bg-surface-container-low">
      ...
      <main className="relative flex-grow space-y-8 overflow-x-hidden p-4 pb-24 sm:p-6 lg:p-12 lg:pb-12">
        <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="mb-1 flex flex-wrap items-center gap-3 font-headline text-2xl font-black text-primary md:text-3xl">
          ...
          <div className="flex flex-wrap gap-3">
            <button onClick={refreshDashboard} className="inline-flex items-center gap-2 rounded-xl border border-outline-variant/20 bg-white px-4 py-3 text-sm font-black text-primary">
              <RefreshCw size={17} /> Refresh
            </button>
            <button onClick={exportData} disabled={!adminData} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-black text-white disabled:opacity-50">
        ...
        {showUploadModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-6 backdrop-blur-sm">
            <div className="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-[2rem] bg-white shadow-2xl md:flex-row">
              <div className="shrink-0 bg-primary p-8 text-white md:w-72 md:p-10">
                <h3 className="font-headline text-2xl font-black tracking-tight">Admin Course Creator</h3>
                <p className="mt-3 text-sm text-white/70">Upload official curriculum content for students.</p>
              </div>
              <div className="relative flex-grow space-y-8 overflow-y-auto p-8 md:p-10">
                <button onClick={() => setShowUploadModal(false)} className="absolute right-6 top-6 text-on-surface-variant hover:text-primary">
                  <XCircle size={24} />
                </button>
                <div className="space-y-4">
                ...
                <div className="border-t border-outline-variant/10 pt-6">
                  ...
                  {courseType === 'video' ? (
                    <div onClick={() => document.getElementById('admin-video-upload')?.click()} className="flex cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-outline-variant/20 bg-surface-container-lowest p-8 text-center transition-colors hover:border-primary/50">
                ...
                <div className="flex items-center justify-between border-t border-outline-variant/10 pt-6">
                  <button onClick={() => setShowUploadModal(false)} className="px-4 py-2 font-bold text-on-surface-variant hover:text-primary">Cancel</button>
                  <button disabled={isUploading || !newCourseTitle.trim()} onClick={publishCourse} className="flex items-center gap-2 rounded-xl bg-secondary px-8 py-3 font-black text-white disabled:opacity-50">
            ...
            <div className="w-full max-w-xl rounded-[2rem] bg-white p-6 shadow-2xl md:p-8">
              ...
              <div className="mt-8 flex justify-end gap-3">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-xl bg-secondary px-6 py-3 text-sm font-black text-white disabled:opacity-50"
                >
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around border-t border-outline-variant/10 bg-white px-1 py-2 shadow-[0_-4px_20px_rgba(0,0,0,0.06)] lg:hidden">
        {tabs.map((item) => (
          <button key={item.name} onClick={() => setActiveTab(item.name)} className={cn('flex min-w-[46px] flex-col items-center gap-0.5 rounded-xl px-1.5 py-1.5 transition-all', activeTab === item.name ? 'text-primary' : 'text-on-surface-variant/60')}>
